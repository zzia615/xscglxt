<%@ Page Title="" Language="C#" MasterPageFile="~/MainSite.Master" AutoEventWireup="true" CodeBehind="Xsgl.aspx.cs" Inherits="xscglxt.Xsgl" %>
...
<div class="form-inline mt-2">
    ...
    <a href="XsglAdd.aspx" class="btn btn-primary ml-1">新增新生</a>
</div>
<div class="mt-2">
    <asp:GridView ID="GridView1" runat="server" CssClass="table table-striped" AutoGenerateColumns="False" DataKeyNames="xsbm" DataSourceID="SqlDataSource1" style="border-color:#efefef;" EmptyDataText="没有查询到数据">
        <Columns>
            ...
            <asp:BoundField DataField="rxrq" HeaderText="入学日期" DataFormatString="{0:d}" SortExpression="入学日期" />
            ...
            <asp:TemplateField>
                <ItemTemplate>
                    <asp:HyperLink ID="HyperLink1" runat="server" NavigateUrl='<%#"XsglEdit.aspx?xsbm="+Eval("xsbm") %>' Text="编辑" CssClass="btn btn-info btn-sm"></asp:HyperLink>
                    <%--<asp:HyperLink ID="HyperLink2" runat="server" NavigateUrl="" Text="删除"></asp:HyperLink>--%>
                    <a class="btn btn-danger btn-sm" href="javascript:void(0);" onclick="if(confirm('确定删除学生信息？')){window.location.href='XsglDel.aspx?xsbm=<%# Eval("xsbm") %>';}" >删除</a>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </asp:GridView>
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:xscglxtConnectionString %>" SelectCommand="SELECT * FROM [xsxx]"></asp:SqlDataSource>
</div>
</asp:Content>
