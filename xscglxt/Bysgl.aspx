<%@ Page Title="" Language="C#" MasterPageFile="~/MainSite.Master" AutoEventWireup="true" CodeBehind="Bysgl.aspx.cs" Inherits="xscglxt.Bysgl" %>
...
<div class="form-inline mt-2">
    ...
    <a href="BysglAdd.aspx" class="btn btn-primary ml-1">新增毕业生</a>
</div>
<div class="mt-2">
    <asp:GridView ID="GridView1" runat="server" CssClass="table table-striped" AutoGenerateColumns="False" DataKeyNames="xsbm" DataSourceID="SqlDataSource1" style="border-color:#efefef;" EmptyDataText="没有查询到数据">
        <Columns>
            ...
            <asp:BoundField DataField="byrq" HeaderText="毕业日期" DataFormatString="{0:d}" SortExpression="毕业日期" />
            ...
            <asp:TemplateField>
                <ItemTemplate>
                    <asp:HyperLink ID="HyperLink1" runat="server" NavigateUrl='<%#"BysglEdit.aspx?xsbm="+Eval("xsbm") %>' Text="编辑" CssClass="btn btn-info btn-sm"></asp:HyperLink>
                    <%--<asp:HyperLink ID="HyperLink2" runat="server" NavigateUrl="" Text="删除"></asp:HyperLink>--%>
                    <a class="btn btn-danger btn-sm" href="javascript:void(0);" onclick="if(confirm('确定删除毕业生信息？')){window.location.href='BysglDel.aspx?xsbm=<%# Eval("xsbm") %>';}" >删除</a>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </asp:GridView>
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:xscglxtConnectionString %>" SelectCommand="SELECT * FROM [bysxx]"></asp:SqlDataSource>
</div>
</asp:Content>
